...
export async function POST(request: Request) {
  try {
    const { poseData, requestType } = await request.json()

    // Handle AI analysis request type
    if (requestType === 'gemini-analysis' && poseData) {
      return await handleAIAnalysis(poseData)
    }
...
async function handleAIAnalysis(poseData: any[]) {
  const phaseAnalysis = analyzeSwimmingPhases(poseData);
  const apiKey = process.env.OPENAI_API_KEY
  
  if (!apiKey) {
    console.warn("OPENAI_API_KEY not configured - returning analysis without AI feedback")
    return NextResponse.json({
      feedback: "Unable to fetch AI feedback. API key not configured.",
      success: true,
  ...
  try {
    const analysis = await generateDetailedAIFeedback(poseData, apiKey)
    return NextResponse.json({
      feedback: analysis,
      success: true,
      phaseAnalysis: phaseAnalysis,
    ...
    return NextResponse.json({
      feedback: "Unable to fetch AI feedback. Please check your API key configuration.",
      success: true,
      phaseAnalysis: phaseAnalysis,
      aiAvailable: false
    })
...
async function generateDetailedAIFeedback(poseData: any[], apiKey: string): Promise<string> {
  ...
  const response = await fetch(
    "https://api.openai.com/v1/chat/completions",
    {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini", // or "gpt-4o" for more advanced analysis
        messages: [
          {
            role: "system",
